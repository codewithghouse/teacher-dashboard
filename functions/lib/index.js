  ...
  "actions": [
    {
      ...
      "subStatus": "Short English label"
        ...
        const pastTestsArr = Array.isArray(payload?.pastTests) ? payload.pastTests : [];
        ...
        if (withPaper.length > 0) {
            const chunks = await Promise.all(withPaper.map(async (t) => {
                try {
                    ...
                    if (!isPdf)
                        return ""; // only PDFs are text-extractable here; skip images/docs
                    const parsed = await pdfParse(Buffer.from(await res.arrayBuffer()));
                    let txt = String(parsed?.text || "").trim();
                    if (!txt)
                        return "";
                    if (txt.length > MAX_PAST_PAPER_CHARS)
                        txt = txt.slice(0, MAX_PAST_PAPER_CHARS);
        ...
        const pastTestsMeta = pastTestsArr.map(t => `- ${t.testName || "Test"}${t.subject ? ` (${t.subject})` : ""}${t.testDate ? ` [${t.testDate}]` : ""}: ${Array.isArray(t.topics) && t.topics.length ? t.topics.join(", ") : "no topic tags"}${t.blueprintUrl ? " · paper attached" : ""}`).join("\n");
        systemPrompt = [
