...
exports.getBranchWeeklyInsight = exports.getOwnerBranchInsight = exports.getPrincipalInsight = void 0;
...
function newOpenAI() {
    return new openai_1.default({ apiKey: openaiApiKey.value().trim() });
}
async function chatJson(client, opts) {
...
// ════════════════════════════════════════════════════════════════════════════
function buildPrincipalTopPrompt(p, network) {
...
function buildPrincipalLowerPrompt(p, top, network, rank) {
...
function sanitizePrincipalInsight(parsed, fallbackLabel) {
...
exports.getPrincipalInsight = functions
...
// ════════════════════════════════════════════════════════════════════════════
function buildBranchTopPrompt(b, network) {
...
function buildBranchLowerPrompt(b, top, network, rank) {
    const atRiskPct = b.studentCount > 0 ? (b.activeAlerts / b.studentCount) * 100 : 0;
...
function sanitizeBranchTop(parsed) {
...
function sanitizeBranchLower(parsed) {
...
exports.getOwnerBranchInsight = functions
    .runWith({ secrets: [openaiApiKey], timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
    requireOwner(context);
    ...
    try {
        ...
        const insight = isTop ? sanitizeBranchTop(parsed) : sanitizeBranchLower(parsed);
...
// ════════════════════════════════════════════════════════════════════════════
function buildBranchWeeklyPrompt(branch, network) {
...
function sanitizeBranchWeeklySection(arr, max = 4) {
...
exports.getBranchWeeklyInsight = functions
    .runWith({ secrets: [openaiApiKey], timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
    requireOwner(context);
    ...
    const systemPrompt = "You are a senior school network analyst. Read the branch's current metrics, " +
