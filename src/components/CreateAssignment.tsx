  ...
  return (
    ...
    <div className="md:hidden" style={{ fontFamily: MA.FONT, background: "#EEF4FF", minHeight: "100vh", margin: "-16px -16px 0", paddingBottom: 158 }}>
      ...
      <div className="px-4 pt-[14px] flex flex-col gap-[12px]">
        ...
        {/* Due Date */}
        <div className="rounded-[18px] px-[14px] pt-[14px] pb-[10px]" style={{ background: MA.CARD, boxShadow: MA.SH }}>
          <label className="block text-[9px] font-bold uppercase mb-[10px] flex items-center gap-[6px]" style={{ color: MA.T3, letterSpacing: "1.5px" }}>
            Due Date <span className="text-[11px] font-bold" style={{ color: MA.RED }}>*</span>
          </label>
          <div ref={datePickerRefMobile} style={{ position: 'relative' }}>
            <div
              role="button"
              tabIndex={0}
              onClick={() => {
                if (formData.dueDate) {
                  const [y, m] = formData.dueDate.split('-').map(Number);
                  setPickerMonth(new Date(y, m - 1, 1));
                } else { setPickerMonth(new Date()); }
                setDatePickerOpen(o => o === "mobile" ? null : "mobile");
              }}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setDatePickerOpen(o => o === "mobile" ? null : "mobile"); } }}
            ...
            {datePickerOpen === "mobile" && (
              <DatePopover
                month={pickerMonth}
                onMonthChange={setPickerMonth}
                value={formData.dueDate}
                onSelect={(d) => { setFormData({ ...formData, dueDate: d }); setDatePickerOpen(null); }}
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 flex flex-col gap-5 ca-enter">
            ...
            <div className="ca-card3d rounded-[22px] p-6"
              ...
              <div ref={datePickerRefDesktop} style={{ position: 'relative' }}>
                <div
                  role="button"
                  tabIndex={0}
                  onClick={() => {
                    ...
                    setDatePickerOpen(o => o === "desktop" ? null : "desktop");
                  }}
                  onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setDatePickerOpen(o => o === "desktop" ? null : "desktop"); } }}
...
// ─── Custom DatePopover ──────────────────────────────────────────────────────
// Replaces the native <input type="date"> popup which was overflowing the
// dashboard card on mobile. This popover renders as a contained absolute child
// of its trigger so it sticks to the card boundary and follows the page theme.
type MAType = {
  CARD: string; SURFACE: string; P: string;
  T1: string; T3: string; T4: string;
  ORANGE: string; FONT: string;
};
const DatePopover = ({
  month, onMonthChange, value, onSelect, onClose, MA,
}: {
  month: Date;
  ...
  const prevMonth = () => onMonthChange(new Date(y, m - 1, 1));
  const nextMonth = () => onMonthChange(new Date(y, m + 1, 1));
  return (
    <div
      role="dialog"
      ...
      style={{
        ...
        background: MA.CARD, borderRadius: 16, padding: 12, zIndex: 60,
        boxShadow: "0 0 0 0.5px rgba(0,85,255,.12), 0 6px 22px rgba(0,40,120,.16), 0 18px 44px rgba(0,40,120,.18)",
        fontFamily: MA.FONT, maxWidth: 320, marginInline: "auto",
      }}
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: MA.T1, letterSpacing: "-0.2px" }}>{monthLabel}</div>
        <div style={{ display: "flex", gap: 4 }}>
          <button type="button" aria-label="Previous month" onClick={prevMonth} style={{ width: 28, height: 28, borderRadius: 8, border: "none", background: MA.SURFACE, color: MA.T3, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
          </button>
          <button type="button" aria-label="Next month" onClick={nextMonth} style={{ width: 28, height: 28, borderRadius: 8, border: "none", background: MA.SURFACE, color: MA.T3, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 4 }}>
        {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => (
          <div key={d} style={{ textAlign: "center", fontSize: 10, fontWeight: 700, color: MA.T3, padding: "4px 0", letterSpacing: "0.6px" }}>{d}</div>
        ))}
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isSel ? MA.P : isToday ? "rgba(0,85,255,0.08)" : "transparent",
                color: isSel ? "#fff" : isPast ? MA.T4 : inMonth ? MA.T1 : MA.T4,
              ...
              onMouseEnter={e => { if (!isSel && !isPast) (e.currentTarget as HTMLButtonElement).style.background = "rgba(0,85,255,0.06)"; }}
              onMouseLeave={e => { if (!isSel && !isPast) (e.currentTarget as HTMLButtonElement).style.background = isToday ? "rgba(0,85,255,0.08)" : "transparent"; }}
            >
              {date.getDate()}
            </button>
          );
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 8, borderTop: "0.5px solid rgba(0,85,255,0.08)" }}>
        <button type="button" onClick={onClose} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "transparent", color: MA.T3, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          Cancel
        </button>
        <button type="button" onClick={() => onSelect(todayStr)} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: MA.SURFACE, color: MA.P, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          Today
        </button>
      </div>
    </div>
